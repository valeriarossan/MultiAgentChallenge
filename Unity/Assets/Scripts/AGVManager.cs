...
public class AGVManager : MonoBehaviour
{
    ...
    [SerializeField] private List<Transform> agvs;
    ...
    private Dictionary<int, Transform> agvObjects = new Dictionary<int, Transform>();
    ...
    private void ApplyAGV(AgvData data)
    {
        if (!agvObjects.ContainsKey(data.id)){
            Debug.LogWarning("No Unity AGV found for ID " +  data.id);
            return;
        }

        Transform agv = agvObjects[data.id];
        Vector3 newTargetPosition = CellToUnity(data.pos[0], data.pos[1], agv.position.y);
        ...
        palletManager.UpdatePalletCarrier(data.id, data.pallet_id);
    }
